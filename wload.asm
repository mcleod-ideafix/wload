...
ResetUART                    Print
                             ...
                             ATSend
                             db "AT+CWAUTOCONN=0",CR,LF,NULL   ;disable AP autoconn on reset
                             WaitResponse
                             db "OK",CR,LF,NULL
                             db NULL
                             ...
                             Print
                             db "Connecting to ",NULL
                             call ReadWifiConfig               ;read /SYS/CONFIG/IW.CFG and connect to AP as configured there
                                                               ;A=0 if success. =1 or 2 if error issuing command. =3 if file not found
                             cp 0
                             jr z,OKWifiConnect
...
ManageCloseConnection        Print
                             ...
                             jp 1ffbh                           ;exit ESXDOS and jump to code at HL
...
ExitByError                  ld hl,Log                          ;if exiting due to network failure
                             ...
                             ld hl,WifiErrorMsg                 ;if exiting duw to IW.CFG not found
...
ReadWifiConfig               proc
                             xor a                               ;
                             rst 8                               ;Get current drive in A
                             db M_GETSETDRV                      ;

                             ld hl,WifiConfigFile                ;
                             ld b,FA_READ                        ;Attempt to open /SYS/CONFIG/IW.CFG
                             rst 8                               ;CF=1 if not found or unreadable
                             db F_OPEN                           ;
                             jr c,FileNotFound                   ;

                             ld hl,Log                           ;
                             ld bc,160                           ;We will use the Log buffer as
                             rst 8                               ;temporary buffer for this read operation
                             db F_READ                           ;Read 160 bytes: 80 for SSID string, 80 for password string

                             rst 8                               ;Close file
                             db F_CLOSE                          ;

                             ld hl,Log                           ;
                             call PrintStringHL                  ;Prints the SSID we are going to connect to
                             ld a,CR                             ;
                             rst 10h                             ;
...
FileNotFound                 Print
                             db CR,"/SYS/CONFIG/IW.CFG not found. Please, run NETMAN.TAP",CR,NULL
                             ld a,3
                             ret
...
WifiConfigFile               db "/SYS/CONFIG/IW.CFG",NULL
